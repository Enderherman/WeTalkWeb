...
- 17 项认证/资料/密码单元测试通过；类型检查和生产构建通过。另通过 Vite 代理对本机真实 Spring Boot、MySQL 和 Redis 完成验证码、注册、登录、资料读取、密码更新、旧密码拒绝、新密码重登、登出和旧 token 失效验证，并清理了临时测试账号及 Redis 测试键。尚未完成自动化浏览器端到端和多视口视觉验收。
