...
- 已接入同源 WebSocket、5 秒心跳、断线重连、INIT 会话初始化、一对一文字收发、历史游标分页和纯文字 IndexedDB 缓存。
- 39 项认证/资料/密码/路由/聊天/缓存单元测试通过；类型检查和生产构建通过。另通过 Vite 代理对本机真实 Spring Boot、MySQL、Redis 和 Netty WebSocket 完成认证密码闭环、双账号好友申请、文字发送/实时接收、HTTP 过期码 901、INIT 消息和心跳键清理验证。临时账号、好友关系、会话、消息及 Redis 键已清理；尚未完成自动化浏览器端到端和多视口视觉验收。
