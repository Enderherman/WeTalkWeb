...
- 已接入同源 WebSocket、5 秒心跳、断线重连、INIT 会话初始化、一对一文字收发、历史游标分页、纯文字 IndexedDB 缓存和消息时间分隔。
- 56 项认证/资料/密码/路由/聊天/缓存/时间分隔/发送状态/API 错误页/OpenAPI 契约单元测试通过；类型检查和生产构建通过。另通过 Vite 代理对本机真实 Spring Boot、MySQL、Redis 和 Netty WebSocket 完成认证密码闭环、双账号好友申请、文字发送/实时接收、发送成功响应与接收方历史读回、HTTP 过期码 901、INIT 消息、心跳键清理，以及服务端断开/恢复时的页面切换验证。Phase 0 接口抽查确认验证码结构、验证码失败响应、五个账号/聊天端点、九个联系人端点、七个群接口和两个文件接口未登录时返回 901；未创建账号、群组、文件或消息。尚未完成自动化浏览器端到端和多视口视觉验收。
...
- `docs/openapi.web.json` 使用 OpenAPI 3.2.1，覆盖网页已接入的 26 个认证、联系人、群聊、文件和一对一文字聊天接口。
- 文档说明 `/api` 同源前缀、`token` 请求头、URL-encoded 表单、业务响应码和当前登录密码 MD5 兼容协议。
- 管理、靓号、版本更新 API，以及 WebSocket 帧说明仍在 Phase 0 待办中。
