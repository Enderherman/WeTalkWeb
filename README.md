...
- 已接入同源 WebSocket、5 秒心跳、断线重连、INIT 会话初始化、一对一文字收发、历史游标分页、纯文字 IndexedDB 缓存、消息时间分隔，以及联系人搜索、好友申请收件箱、好友目录、资料查看和关系管理；收到实时申请帧时更新侧栏计数。
- 84 项认证/资料/密码/路由/聊天/缓存/联系人/API 错误页/OpenAPI 契约单元测试通过；类型检查和生产构建通过。Vite 代理和临时本地账号完成认证密码闭环、联系人搜索/申请/同意、好友资料、删除/拉黑、双账号文字发送与接收、历史读回、WebSocket INIT/心跳、HTTP 过期码 901 和服务端断开/恢复验证；测试账号和关联数据已清理。Phase 0 接口抽查确认验证码结构、验证码失败响应、账号设置/资料保存、账号/聊天、联系人、群和文件端点未登录时返回 901。管理员角色与用户列表敏感字段的真实运行时回归仍待使用有效账号；尚未完成自动化浏览器端到端和多视口视觉验收。
...
- 当前已有文字消息展示与发送、历史分页、IndexedDB 缓存、联系人搜索/申请、申请收件箱和好友目录管理；自动化浏览器端到端与群聊管理仍列入后续阶段。
